...
*/				
	...
	public int pick(int slot, int side) {
		 ...
		 */
		if (slot > myStore || slot < myStore-6 || board[slot] == 0) return side*7 - 1;
		else {
			...
			while (grab > 0) {
				pos++;;
				if ((side == 1 && pos == 13)||(side == 2 && pos == 6)||(pos == slot)) pos++;
				pos = pos % 14;
				grab--;
				board[pos]++;
			}
	...
	public boolean checkCapture(int slot, int side) {
		...
		if (board[slot] == 1 && slot < myStore && slot > myStore-6) {
			if (slot < 6)	opposite = slot + (6-slot)*2;
			else 			opposite = slot - (slot-6)*2;
			captured += board[slot];
	...
	public void printBoard() {
		int i;
		System.out.print("---- ");
		for (i = 12; i > 6; i--) System.out.printf("%02d ", board[i]);
		System.out.println("----");
		
		System.out.printf(" %02d ", board[13]);
		for (i = 0; i < 18; i++) System.out.printf("=");
		System.out.printf("  %02d\n", board[6]);
		
		System.out.print("---- ");
		for(i = 0; i < 6; i++) System.out.printf("%02d ", board[i]);
		System.out.println("----");
	}
